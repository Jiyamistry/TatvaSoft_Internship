    ...
    public class DALLogin
    {
            ...
            public string LoginUserProfileUpdate(UserDetail userDetail)
            {
                    ...
                    using (var transaction = _cIDbContext.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            if (existingUserDetail != null)
                            {
                                ...
                                existingUserDetail.Name = userDetail.FirstName ?? existingUserDetail.Name; // Ensure not null
                                existingUserDetail.Surname = userDetail.LastName ?? existingUserDetail.Surname; // Ensure not null
                            ...
                            if (user != null)
                            {
                                ...
                                user.FirstName = userDetail.FirstName ?? user.FirstName; // Ensure not null
                                user.LastName = userDetail.LastName ?? user.LastName; // Ensure not null
                                //user.PhoneNumber = userDetail.PhoneNumber;
                                user.UserType = userDetail.UserType;
                        ...
                        catch (Exception ex)
                        {
                            ...
                            // Log the error message (using a logging framework or console for simplicity)
                            Console.WriteLine($"Error: {innerExceptionMessage}");
                            throw new Exception($"An error occurred while saving the entity changes. Inner exception: {innerExceptionMessage}", ex);
                        }
                    }
                }
                catch (Exception ex)
                {
                    ...
                    var outerExceptionMessage = ex.InnerException != null ? ex.InnerException.Message : ex.Message;
                    // Log the error message (using a logging framework or console for simplicity)
                    Console.WriteLine($"Error: {outerExceptionMessage}");
                    throw;
                }
                return result;
            }
